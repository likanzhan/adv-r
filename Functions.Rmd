...
Base R provides two ways to compose multiple function calls. For example, imagine you wanted to compute the population standard deviation from its underlying building blocks, `sqrt()`, `mean()`:
...
You could nest the function calls:
...
The magrittr package provdes `%>%` (called the pipe, and usually pronounced "and then"):
